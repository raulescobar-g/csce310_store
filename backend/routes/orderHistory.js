...
router.post('/addorder', async (req, res) => {
    try {
        ...
        const { payment_id, user_id, product_id, order_num } = req.body
        if (!user_id || !product_id || !payment_id || !order_num) {
            res.sendStatus(400)
            return
        }
        const result = await req.app.get('pool').query(`INSERT INTO order_history (payment_id, user_id,product_id, order_num, quantity) VALUES ($1, $2, $3, 1, 2));`, [payment_id, user_id, product_id])
 ...
 * joins order_history with cart on product_id and returns list items that are not bought
 * If missing value will return 400 status
...
router.get('/', async (req, res) => {
    try {
        ...
        const history_arr = await req.app.get('pool').query("SELECT payment_id, user_id, card_type, card_number,card_expiration_month, card_expiration_month, card_expiration_year, card_cvv, cardholder_firstname, cardholder_lastname, billing_address_one, billing_address_city, billing_address_state, billing_address_zip, quantity FROM payment JOIN cart ON order_history.cart_id=cart.cart_id where user_id=$1", [user_id])
